...
function Dashboard() {
  ...
  const nonTransferTransactions = useMemo(() => {
    if (!data) return [];
    return data.transactions.filter((t: any) => t.type !== "transfer");
  }, [data]);

  const income = nonTransferTransactions
    .filter((t: any) => t.type === "income")
    .reduce((s: number, t: any) => s + Number(t.amount), 0);
  const expense = nonTransferTransactions
  ...
  const portfolioValue = useMemo(() => {
    ...
    for (const a of data.assets) {
      const aData = data.accounts.find((acc: any) => acc.id === a.account_id);
      // Apenas considera ativos vinculados a contas que não são cartões de crédito
      if (aData && aData.type !== "credit_card") {
...
<dyad-chat-summary>Ajustando cálculo de receita/despesa para excluir transferências</dyad-chat-summary>
A lógica de cálculo de receita e despesa no dashboard foi atualizada para excluir transações do tipo "transferência", garantindo que apenas receitas e despesas reais afetem os totais exibidos.<dyad-write path="src/routes/app.dashboard.tsx" description="Ajustando o cálculo de receita/despesa para excluir transferências.">
import { createFileRoute, Link } from "@tanstack/react-router";
import { useMemo, useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
...
function Dashboard() {
  ...
  const nonTransferTransactions = useMemo(() => {
    if (!data) return [];
    // Considera transações que não são do tipo 'transfer'
    return data.transactions.filter((t: any) => t.type !== "transfer");
  }, [data]);
