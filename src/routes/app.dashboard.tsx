...
function Dashboard() {
  ...
  // Filtra transações que não são transferências internas para o cálculo de receita/despesa
  const filteredTransactionsForBalance = useMemo(() => {
    if (!tx) return [];
    return tx.filter((t: any) => t.type !== "transfer");
  }, [tx]);

  const income = filteredTransactionsForBalance.filter((t: any) => t.type === "income").reduce((s: number, t: any) => s + Number(t.amount), 0);
  const expense = filteredTransactionsForBalance.filter((t: any) => t.type === "expense").reduce((s: number, t: any) => s + Number(t.amount), 0);
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      >
        <span className="h-8 w-8 rounded-lg bg-gradient-primary flex items-center justify-center text-primary-foreground shrink-0">
        ...
        <DialogContent className="max-w-3xl w-[95vw] h-[85vh] p-0 gap-0 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-border bg-surface-1/50">
            <div className="flex items-center gap-2">
              <span className="h-7 w-7 rounded-md bg-gradient-primary flex items-center justify-center text-primary-foreground">
              ...
              {data?.openInvoices.map((inv: any) => {
                ...
                return (
                  <div key={inv.id} className="flex items-center justify-between p-3 rounded-lg bg-surface-2 border border-border">
                    ...
                    <div className="font-mono tabular text-expense whitespace-nowrap">{formatBRL(Number(inv.total_amount || 0))}</div>
                  </div>
                );
              })}
            </div>
...
<dyad-chat-summary>Ajustando cálculo de saldo mensal para excluir transferências</dyad-chat-summary>
O cálculo de Receita e Despesa do mês no Dashboard foi ajustado para não incluir transferências internas, garantindo que apenas transações externas afetem o saldo.<dyad-write path="src/routes/app.dashboard.tsx" description="Ajustando o cálculo de receita e despesa para excluir transferências internas.">
import { createFileRoute, Link } from "@tanstack/react-router";
import { useMemo, useState } from "react";
import { useQuery } from "@tanstack/react-query";
import { supabase } from "@/integrations/supabase/client";
...
function Dashboard() {
  ...
  const filteredTransactionsForBalance = useMemo(() => {
    if (!tx) return [];
    // Considera apenas transações que não são do tipo 'transfer'
    return tx.filter((t: any) => t.type !== "transfer");
  }, [tx]);
