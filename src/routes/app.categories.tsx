...
function CategoriesPage() {
  ...
  const { data: cats = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from<any>("categories").select("*").order("kind").order("name");
  ...
  const create = async () => {
    ...
    const { error } = await supabase.from<any>("categories").insert({
  ...
  const saveEdit = async (id: string) => {
    const { error } = await supabase.from<any>("categories").update({ name: editForm.name, icon: editForm.icon }).eq("id", id);
    if (error) toast.error(error.message);
  ...
  const remove = async (id: string) => {
    ...
    const { error } = await supabase.from<any>("categories").delete().eq("id", id);
    if (error) toast.error(error.message);
