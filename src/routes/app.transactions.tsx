...
const recomputeInvoiceTotal = async (invoiceId: string) => {
  // ✅ Fix: Type the select results so `amount` is known
  const { data: txData, error: txErr } = await supabase
    .from<{ amount: number }>("transactions")
    .select("amount")
    .eq("invoice_id", invoiceId);
  if (txErr) throw txErr;
  const txTotal = (txData || []).reduce((sum, tx) => sum + Number(tx.amount), 0);

  const { data: itemsData, error: itemsErr } = await supabase
    .from<{ amount: number }>("invoice_items")
    .select("amount")
    .eq("invoice_id", invoiceId);
  if (itemsErr) throw itemsErr;
  const itemsTotal = (itemsData || []).reduce((sum, item) => sum + Number(item.amount), 0);
  const total = txTotal + itemsTotal;

  // ✅ Fix: Type the update result so `total_amount` is known
  await supabase
...
function TxPage() {
  ...
  // (only the recomputeInvoiceTotal function was modified)
  // Keep the rest of the file as‑is to avoid breaking existing logic.
  // The errors above are resolved by the generic type parameters added to supabase calls.
