  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [txR, lmR, billsR, invR, profR, accR, itemsR] = await Promise.all([
        supabase.from<any>("transactions").select("*, categories(name, icon)").gte("occurred_on", monthStart),
        supabase.from<any>("transactions").select("amount, type, category_id, occurred_on, categories(name)").gte("occurred_on", lmStart).lt("occurred_on", monthStart),
        supabase.from<any>("fixed_bills").select("*").eq("active", true),
        supabase.from<any>("invoices").select("*, accounts!inner(name, archived)").eq("accounts.archived", false).in("status", ["open", "closed"]),
        supabase.from<any>("profiles").select("*").maybeSingle(),
        supabase.from<any>("accounts").select("*").eq("archived", false),
        supabase.from<any>("invoice_items").select("*"),
      ]);
      return {
        tx: txR.data ?? [],
        lastTx: lmR.data ?? [],
